  ...
  const activity_format = (inputForm, type) => {
    ...
    for (let i = 1; i <= Object.keys(inputForm).length / columns; i++) {
      ...
      if (type === '1') {
        ...
        const duration = Math.round((tiempoOptimista + 4 * tiempoMedio + tiempoPesimista) / 6);
        const variance = Math.pow((tiempoPesimista - tiempoOptimista) / 6, 2);
        tasks.push({ name, duration, dependencies, variance, tiempoOptimista, tiempoMedio, tiempoPesimista });
      }
    }
    return tasks
  }
